public class CTR {
	...
	public static String ctr_decode_mode(String[] blocksToDecode, String k, String yMinus1) {
		String result = "";
		int l = blocksToDecode[0].length();

		// xi = E((y_-1 + i) mod 2^l, k) XOR yi (i = 0, ..., n-1)
		for (int i = 0; i < blocksToDecode.length; i++) {
			result += Bonustask.xor(ctr_E(yMinus1, i, l, k), blocksToDecode[i]);
		}
		return result;
	}
	...
	public static String ctr_encode_mode(String[] blocksToEncode, String k, String yMinus1) {
		...
		for (int i = 0; i < blocksToEncode.length; i++) {
			result += Bonustask.xor(ctr_E(yMinus1, i, l, k), blocksToEncode[i]);
		}

		return result;
	}

	private static String ctr_E(String yMinus1, int i, int l, String k) {
		// (y_-1 + i)
	...
	private static String binaryPadding(String s, int l) {
		String padding = "";
		for (int i = 0; i < l; i++)
			padding += "0";
